...
nnoremap  ( ?(v%
nnoremap  U U
vnoremap  ra y:%s/"//gc<Left><Left><Left>
vnoremap  r y:s/"/
vnoremap  f y/"
...
nnoremap  O Oj
nnoremap  o ok
nnoremap  ra yiw:%s/"//gc<Left><Left><Left>
nnoremap  f yiw:/\v"
noremap  t viwpyiw
nnoremap  v viw
nnoremap  y yiw
...
nnoremap  P PLOG()<<;<Left>
nnoremap  D DLOG()<<;<Left>
nnoremap  VV :tabnew ~/.vimrc.bundles
nnoremap  z :tabnew ~/todo_and_doing_list.txt
map  zz :call ToggleFold()
nnoremap  r yiw:s/"/
noremap  wb :TagbarToggle
map  wt <Plug>NERDTreeTabsToggle
...
nmap  en <Plug>LocationNext
nmap  ep <Plug>LocationPrevious
nnoremap  el :call ToggleErrors()
nnoremap  F viwy:Unite grep:."
nnoremap  p :YRShow
nnoremap  s :AT
nnoremap  cf :Autoformat
nnoremap # #zzzv
...
noremap ,j [[
noremap ,h []
nnoremap <silent> ,C :GscopeFind s jj
nnoremap <silent> ,c :GscopeFind c jj
...
xnoremap <silent> p :YRPaste 'p', 'v'
nnoremap <silent> p :YRPaste 'p'
nnoremap t :execute 'tabnext '.g:last_active_tab
noremap t0 :tablast
noremap t9 9gt
noremap t8 8gt
noremap t7 7gt
...
silent only
cd ~/xndm_work/src/samh_ticket_rank_0.1
if expand('%') == '' && !&modified && line('$') <= 1 && getline(1) == ''
  let s:wipebuf = bufnr('%')
endif
set shortmess=aoO
badd +1 conf_ticket_rank.yaml
badd +0 docker-compose.yaml
badd +0 istio-config.yaml
badd +0 Dockerfile
badd +0 Makefile
argglobal
silent! argdel *
argadd conf_ticket_rank.yaml
argadd docker-compose.yaml
argadd istio-config.yaml
argadd Dockerfile
argadd Makefile
edit conf_ticket_rank.yaml
...
setlocal tabstop=4
setlocal tagcase=
setlocal tags=~/.cache/tags/root-xndm_work-src-samh_ticket_rank_0.1-.tags,./tags,./TAGS,tags,TAGS
...
9
normal! zo
let s:l = 74 - ((62 * winheight(0) + 32) / 64)
if s:l < 1 | let s:l = 1 | endif
exe s:l
normal! zt
74
